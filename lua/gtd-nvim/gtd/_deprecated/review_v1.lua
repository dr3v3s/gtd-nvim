...
local function get_calendar_events(days_offset, days_count)
  ...
  if not chronos_client then
    return { "(Chronos not available)" }
  end
  
  local raw_events = nil
  
  if days_offset < 0 then
    -- PAST EVENTS: Use Chronos "past" command
    raw_events = chronos_client.calendar_past(math.abs(days_offset))
  else
    -- FUTURE EVENTS: Use Chronos "upcoming" command
    raw_events = chronos_client.calendar_upcoming()
    if raw_events then
