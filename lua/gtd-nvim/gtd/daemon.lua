-- ============================================================================
-- GTD-NVIM CHRONOS DAEMON INTEGRATION
-- ============================================================================
-- Integration with Chronos daemon for real-time GTD metrics, calendar, and reminders
-- Single source of truth for all external data
--
-- @module gtd-nvim.gtd.daemon
-- @version 1.1.0
-- @see ~/Developer/chronos (Go daemon source)
-- @see ~/Developer/chronos-bridge (Swift EventKit CLI)
-- ============================================================================
...
M.config = {
  ...
  bridge_socket_path = vim.fn.expand("~/.cache/chronos/chronos-bridge.sock"),
...
--- Check if chronos-bridge socket exists
---@return boolean
function M.is_bridge_available()
  return file_exists(M.config.bridge_socket_path)
end
...
function M.status()
  ...
  return {
    ...
    bridge_socket_path = M.config.bridge_socket_path,
...
---@param use_bridge boolean|nil Use chronos-bridge instead of chronosd
---@return table|nil, string|nil Response data or nil, error message
function M.query(module, cmd, params, use_bridge)
  local socket = use_bridge and M.config.bridge_socket_path or M.config.socket_path
  
  if not file_exists(socket) then
    return nil, (use_bridge and "chronos-bridge" or "chronosd") .. " not available"
  end
...
--- Query chronos-bridge specifically (for EventKit data)
---@param module string Provider name (calendar, reminders, mail)
---@param cmd string Command to execute
---@param params table|nil Optional parameters
---@return table|nil, string|nil Response data or nil, error message
function M.bridge_query(module, cmd, params)
  return M.query(module, cmd, params, true)
end
...
--- Async query to Chronos daemon
---@param module string Provider name
---@param cmd string Command
---@param params table|nil Parameters
---@param callback function Callback(data, err)
---@param use_bridge boolean|nil Use chronos-bridge instead of chronosd
function M.query_async(module, cmd, params, callback, use_bridge)
  local socket = use_bridge and M.config.bridge_socket_path or M.config.socket_path
  
  if not file_exists(socket) then
    vim.schedule(function()
      callback(nil, (use_bridge and "chronos-bridge" or "chronosd") .. " not available")
    end)
    return
  end
...
function M.calendar_metrics()
  local data, err = M.bridge_query("calendar", "metrics")
  if data then
    return data, nil
  end
...
function M.calendar_today()
  local data, err = M.bridge_query("calendar", "today")
  if data then
    return data, nil
  end
  return nil, err
...
function M.calendar_upcoming(days)
  ...
  return M.bridge_query("calendar", "upcoming", params)
end

--- Get next calendar event
---@return table|nil Single event or nil
function M.calendar_next()
  return M.bridge_query("calendar", "next")
end
...
function M.calendar_past(days)
  return M.bridge_query("calendar", "past", { days = days or 7 })
end
...
function M.reminders_metrics()
  local data, err = M.bridge_query("reminders", "metrics")
  if data then
    return data, nil
  end
...
function M.reminders_all()
  return M.bridge_query("reminders", "all")
end
...
function M.mail_unread()
  local data, err = M.bridge_query("mail", "metrics")
  if data then
    return data, nil
  end
...
function M.setup(user_config)
  ...
  M.config.bridge_socket_path = vim.fn.expand(M.config.bridge_socket_path)
  M.config.cache_dir = vim.fn.expand(M.config.cache_dir)
  
  -- Create user commands
  vim.api.nvim_create_user_command("ChronosDaemonStatus", function()
    local status = M.status()
    local lines = {
      "Chronos Daemon Status",
      "═════════════════════",
      "chronosd:       " .. (status.available and "✓ running" or "✗ not running"),
      "chronos-bridge: " .. (status.bridge_available and "✓ running" or "✗ not running"),
      "",
      "Socket:        " .. status.socket_path,
      "Bridge Socket: " .. status.bridge_socket_path,
...
-- Legacy alias - prefer using require("gtd-nvim.gtd.daemon") directly
M.chronos = M  -- self-reference for compatibility
