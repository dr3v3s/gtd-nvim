...
-- Focus-mode / Sketchybar integration (Notes mode)
...
local function get_gtd_tasks()
  ...
  local cmd = string.format(
    'rg -n "^\\*+\\s+(TODO|NEXT|WAITING|DONE|PROJ|SOMEDAY|MAYBE)" %s --type org 2>/dev/null | head -200',
...
function M.setup(opts)
  ...
  -- Focus-mode integration: whenever you enter a note under ~/Documents/Notes, set mode = "zk"
  if focus_mode then
    vim.api.nvim_create_autocmd("BufEnter", {
      pattern = "*.md",
      callback = function(args)
        local path = vim.fn.fnamemodify(args.file or vim.fn.expand("<afile>:p"), ":p")
        if path:sub(1, #cfg.notes_dir) == cfg.notes_dir then
